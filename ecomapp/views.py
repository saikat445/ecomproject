...
class CheckoutView(EcomMixin, CreateView):
    ...
    def razorpay (self, request, *args, **kwargs):
        o_id = request.GET.get("o_id")
        order = Order.objects.get(id=o_id)

        client = razorpay.Client(auth=("rzp_test_Ufk0RO8BrKnrTI", "6EhfQlYS9K8O8hpXTzCd4H0T"))
        payment = client.order.create({"amount":100*100, "currency": "INR", "payment_capture": 1,})
        ...
        print(payment)
        print('*********')
        #return render( request, "RazorpayRequestView.html", payment)
        context = {'cart':cart_obj.total,'payment':payment}
        return render(request, "checkout.html", payment)
...
class RazorpayRequestView(View):
    def get(self, request, *args, **kwargs):
        ...
        amount = request.GET.get("amount")
        client = razorpay.Client(auth=("rzp_test_Ufk0RO8BrKnrTI", "6EhfQlYS9K8O8hpXTzCd4H0T"))
        payment = client.order.create({"amount":order.total*100, "currency": "INR", "payment_capture": 1,})

        #cart_obj.razor_pay_order_id = payment['id']
        #cart_obj.save
        #return render( request, "RazorpayRequestView.html", payment)
        context = {'cart':order,'payment':payment}
        return render(request, "RazorpayRequestView.html", context)

class RazorpayVerifyView(View):
    def get(self, request, *args, **kwargs):
        payment_id= request.GET.get('razorpay_payment_id')
        Order_id= request.GET.get('razorpay_order_id')
        payment_sign= request.GET.get('razorpay_signature')
        print(payment_id,Order_id,payment_sign)
        #return JsonResponse(data)
